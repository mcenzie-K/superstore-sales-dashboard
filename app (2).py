...
# --- Forecasting Models (trained within app.py) ---
@st.cache_resource
...
# --- Clustering Data (trained within app.py) ---
@st.cache_resource
def prepare_clustering_data(df_input):
    ...
    scaled_features = clustering_scaler.fit_transform(clustering_df)
    ...
    optimal_k = 3
    clustering_kmeans = KMeans(n_clusters=optimal_k, random_state=42, n_init=10)
    clustering_df['Cluster'] = clustering_kmeans.fit_predict(scaled_clustering_df)
    ...
    return clustering_df, pca_df, clustering_scaler, clustering_kmeans

clustering_df, pca_df, clustering_scaler, clustering_kmeans = prepare_clustering_data(df.copy())


# --- HELPER FUNCTIONS FOR FORECASTING ---
def get_next_month_features(last_date, current_df, model, model_features, forecast_horizon):
    future_dates = pd.date_range(start=last_date + pd.DateOffset(months=1), periods=forecast_horizon, freq='MS')
    future_data = []

    # Ensure current_df has the right structure and is indexed by time for lag calculations
    # It needs to resemble the data the model was trained on, including 'Monthly Sales'
    temp_df = current_df.copy().reset_index().set_index('Order_Period') if 'Order_Period' in current_df.columns else current_df.copy()

    # Ensure we only have the columns necessary for the recursive forecasting (Monthly Sales and time features)
    # and that they are in the correct order for generating new features
    # The full xgb_df (overall_xgb_df or segment_data) already contains these.
    # We need to make sure `temp_df` is extended with predicted values to calculate subsequent lags and rolling means.

    for i in range(forecast_horizon):
        current_date = future_dates[i]

        # Prepare features for prediction. These are dynamic and depend on previous actuals/predictions.
        # This requires the temp_df to be growing with predictions.
        lag_1 = temp_df['Monthly Sales'].iloc[-1] if not temp_df.empty else 0
        lag_2 = temp_df['Monthly Sales'].iloc[-2] if len(temp_df) >= 2 else 0
        lag_3 = temp_df['Monthly Sales'].iloc[-3] if len(temp_df) >= 3 else 0

        # Rolling mean calculation should be robust. min(3, len(temp_df)) ensures it handles early periods.
        rolling_mean_val = temp_df['Monthly Sales'].rolling(window=min(3, len(temp_df))).mean().iloc[-1] if not temp_df.empty else 0

        new_features = {
            'Sales_Lag_1': lag_1,
            'Sales_Lag_2': lag_2,
            'Sales_Lag_3': lag_3,
            'Rolling_Mean_3': rolling_mean_val,
        ...
        # Add only the target and time features that the model itself might use, for simplicity.
        # The lag features will be re-calculated in the next iteration based on the updated `temp_df`
        temp_df = pd.concat([
            temp_df,
            pd.DataFrame({
...
def forecast_explorer():
    ...
    # Define the base feature columns for XGBoost models (used in helper functions)
    # This needs to be consistent with what the models were trained on.
    model_feature_names = ['Sales_Lag_1', 'Sales_Lag_2', 'Sales_Lag_3', 'Rolling_Mean_3', 'Month', 'Year', 'Quarter']
    ...
    if forecast_type == "Overall Sales":
        ...
        # Generate forecast using the overall model
        future_forecast_overall = get_next_month_features(last_date_overall, overall_xgb_df, overall_xgb_model, model_feature_names, forecast_horizon_overall)
        ...
        if segment_choice and segment_models[segment_choice] is not None:
            ...
            future_forecast_segment = get_next_month_features(last_date_segment, segment_data, segment_model, model_feature_names, forecast_horizon_segment)
